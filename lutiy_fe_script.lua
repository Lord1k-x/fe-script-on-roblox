--reanimate by Lord1k https://discord.gg/Q6bcgVTNeX
...
--4 - CFrame
local flingpart = "LowerTorso" --name of the part or the hat used for flinging
--the fling function
...
local att1 = gp(gp(gp(c, "Meshes/CRL4Accessory", "Accessory"), "Handle", "BasePart"), "att1_Handle", "Attachment")
att1.Parent = c["RightHand"]
att1.Position = Vector3.new(0, -0.8, -0.3)
att1.Orientation = Vector3.new(180, 90, -90)
...
function KeyD(key)
	...
	if key == hotkey then
		...
		roll.AnimationId = "http://www.roblox.com/asset/?id=705161601"
...
function Light()
    ...
    playerLight = game:GetService("Workspace").HAZER4IK.UpperTorso:FindFirstChild("Light")
    if playerLight then
        playerLight:Destroy()
    else
        light = Instance.new("SurfaceLight",game:GetService("Workspace").HAZER4IK.UpperTorso)
        light.Name = "Light"
